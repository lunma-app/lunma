...
import type { IconName, SavedTabId, TabBoundary, WindowId } from '../shared/types';
...
/** Never render a blank title: fall back to the URL host when the title is empty. */
...
function contextItemsFor(fav: FavView): MenuItem[] {
  ...
  // delete. Some glyphs are generic lucide names (not the curated Space-picker
  // catalogue), cast like TempTabs' `'x' as IconName`.
...
<style>
  ...
  .drop-line-x {
    ...
    z-index: 2;
    pointer-events: none;
    animation: favicon-drop-line-in 120ms var(--ease-emphasised);
  }
  .drop-line-x::before {
    content: '';
    position: absolute;
  ...
  @keyframes favicon-drop-line-in {
    from {
      opacity: 0;

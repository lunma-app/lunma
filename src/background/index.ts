...
import { handleAutoArchiveAlarm, registerAutoArchiveAlarm } from './auto-archive';
...
chrome.runtime.onMessage.addListener((raw: unknown): void => {
  if (!raw || typeof raw !== 'object') return;
...
// `refreshBoundTabBoundaries` no-ops cleanly before boot populates bindings.
...
chrome.runtime?.onInstalled?.addListener(() => {
  void backfillOverlayIntoOpenTabs();
});
...
function registerChromeListeners(): void {
  ...
  chrome.commands?.onCommand.addListener((command) => {
    if (command === 'pin-active-tab') {
      void enqueuePinActiveTab();
      return;
    }
    if (command === 'toggle-launcher') {
      void toggleLauncherOverlay();
      return;
    }
  });

  // Auto-archive (auto-archive): register the fixed 1-minute sweep alarm and its
  // listener. `chrome.alarms.create` is idempotent (replaces the same-named
  // schedule), so re-registering on every boot is safe and survives SW teardown.
  // The onAlarm listener is registered SYNCHRONOUSLY (an alarm fire can be the
  // wake-up message, and MV3 only routes it to first-turn listeners); the handler
  // reads the master switch and enqueues one sweep, DEFERRED to `bootReady` like
  // every other event so the sweep never drains against a half-loaded store.
...
async function openNewTabLauncher(windowId: number | undefined): Promise<void> {
  ...
  await chrome.tabs.create(
    windowId === undefined ? { url, active: true } : { url, windowId, active: true },
  );
}

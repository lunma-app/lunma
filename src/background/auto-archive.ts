 ...
 *
 *   - `registerAutoArchiveAlarm()` / `handleAutoArchiveAlarm` — the fixed
 *     1-minute `chrome.alarms` trigger that enqueues one `autoArchiveSweep`
 *     coordinator command per fire (when the global master switch is on).
...
/** The fixed-period sweep alarm. Its period (1 minute) is distinct from the
 * user-tunable idle threshold (`autoArchiveIdleMinutes`). */
...
/** Register the fixed-period sweep alarm. Idempotent — `chrome.alarms.create`
 * with the same name replaces the prior schedule, so calling this on every SW
 * boot is safe (it survives SW teardown via `chrome.alarms`). */
export function registerAutoArchiveAlarm(): void {
  chrome.alarms.create(AUTO_ARCHIVE_ALARM_NAME, { periodInMinutes: 1 });
}

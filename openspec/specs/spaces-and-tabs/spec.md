...
`src/shared/space-names.ts`. The Space record SHALL store the user's chosen casing
...
**10. On-colour text.** Text rendered on a Space colour — the switcher chip's icon glyph / count and any on-accent label — SHALL use the per-colour `--space-on` ink (dark on light colours, light on dark colours) so it meets WCAG 2.1 AA across the palette's full light→dark range, asserted **per colour** by `src/ui/contrast.test.ts`.
...
`src/launcher/newtab/index.html`, so every tab opened without an explicit URL

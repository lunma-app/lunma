...
or a **lens**
(`{ kind: 'lens'; id: FolderId; name; icon; source; baseUrl; query; refreshMinutes; lensKind }`
— configuration only; its displayed children are ephemeral connector results
owned by the `lenses` capability and are NOT persisted in the node).
Folders are single-level: a folder's `children` holds `SavedTabId` values only
and never nested folders; a lens node has no `children` field. A pinned tab's
...
- **AND** each lens node's configuration fields are restored intact, with no result items read from storage

#### Scenario: Pinned order preserved

- **WHEN** pinned entries (tabs, folders, or lenses) are reordered
...
listed beneath it when expanded; a **lens node** renders as a lens —
...
`lenses` capability) — all in `pinnedBySpace` order.
...
name to the same column as a tab row's title — and a lens's row SHALL
...
NOT cause horizontal overflow. A lens's result rows SHALL use the same
child content inset.
...
- **AND** lens nodes appear as lens rows with icon, name, and item-count badge
...
- **WHEN** a folder's (or lens's) children are shown
...
A **lens** node drags and reorders among pins as one unit, exactly
like a folder node.
...
A lens node is an **inert "onto" target**: the drag controller SHALL
NOT report a lens row via `targetOntoId`, so a pinned or temporary tab
dropped on a lens row lands as a nearest-edge top-level insertion
(exactly as if dropped between rows) — it SHALL NOT file into the lens
...
lens into the temporary section SHALL be rejected exactly like
...
#### Scenario: A drop onto a lens row inserts adjacent, never files or mints

- **WHEN** the user drops a pinned or temporary tab directly onto a lens row
- **THEN** the tab lands as a top-level node at the nearest edge of the lens row's position
- **AND** no folder is created and nothing files "into" the lens node
- **AND** the lens node's config fields are unchanged

#### Scenario: Dragging a lens to temporary is rejected

- **WHEN** the user drops a lens node on the temporary section
- **THEN** no change is made to state
- **AND** the lens animates back to its origin position
...
as the populated grid. This placeholder is the grid's only empty state — it renders
whenever `state.faviconRow` is empty, regardless of the active Space's pinned count.
...
- **GIVEN** the sidebar is mounted with an empty `state.faviconRow` (regardless of whether the active Space has pinned bookmarks)

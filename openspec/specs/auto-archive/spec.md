...
The auto-archive capability SHALL own three settings persisted to `chrome.storage.sync` and declared in the `SETTINGS` registry (`src/shared/settings.ts`) under an `Auto-archive` group:

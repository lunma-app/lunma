...
On a smart folder with ≥ 2 **resolved sections**, each section SHALL be individually collapsible
from its section header, independent of the other sections and of the folder-level expand/collapse.
This applies to **multi-section folders only**; a folder with a single resolved section has no
...
mutator `setSmartSectionCollapsed(windowId, folderId, sourceKey, collapsed): void` SHALL write it.
...
Collapse state SHALL be **per-window**: the same folder's section MAY be collapsed in one window
and expanded in another, with no cross-window write.

When a section is collapsed, the folder SHALL render the section header (with its attention count)
...
(`aria-expanded={!collapsed}`). Collapsing a section SHALL NOT affect the folder-level badge,
...
- **GIVEN** a two-section folder `[gitlab:gitlab.com:authored (ok, 5 items), gitlab:gitlab.com:review-requested (ok, 3 items)]`, both expanded, in window 100
- **WHEN** the user activates the authored section header
- **THEN** `setSmartSectionCollapsed(100, folderId, 'gitlab:gitlab.com:authored', true)` is written
...
- **GIVEN** a folder open in window 100 and window 200, with its authored section collapsed in window 100
...
- **GIVEN** a two-section folder whose collapsed gitlab authored section holds 4 items and whose expanded feed section holds 2 unread
- **WHEN** the folder renders
- **THEN** the folder badge reads `6` (collapse does not change the badge)

#### Scenario: Single-section folders have no per-section collapse

- **GIVEN** a smart folder with exactly one resolved section
- **WHEN** the folder is expanded

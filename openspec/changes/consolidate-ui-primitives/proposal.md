...
- **Docs/spec lockstep:** `docs/architecture.md` component-library list updated (drop `Stack`/`Pill`/`Divider`/`RowButton`/`RowMenu`/`ContextMenu`, add `Menu`); the `tab-row-menu`, `spaces-and-tabs`, and `lunma-bookmark-bindings` specs get menu-rename deltas (below), resolving the pre-existing `ContextMenu`/`RowMenu` → `BitsContextMenu`/`BitsMenu` name drift onto one `Menu`.

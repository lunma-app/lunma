...
`docs/architecture.md` gains the permission/grant model (required vs optional, the SW-queries / surface-requests split, the load-bearing-content-script constraint). The `openspec/specs/` `runtime-permissions` capability is added and notes the smart-folders/launcher degradation. No other `docs/` files change.

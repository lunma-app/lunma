...
Live forge work items inside the pinned section. A lens is a third
...
`lenses` slice, never persisted. An alarms-driven background engine polls
on a per-lens cadence (plus sidebar-open and post-boot refresh kicks),
...
`openUrl` command. Lenses are created and edited from the pinned-header menu

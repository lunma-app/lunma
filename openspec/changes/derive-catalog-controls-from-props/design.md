...
`derive-controls.ts` exports a pure `deriveControls(source: string): DerivedControls` (no filesystem access). Two independent callers:
- `apps/extension/vite.catalog.config.ts` glob-reads every `src/ui/*.svelte` and writes a gitignored `apps/extension/catalog/lib/derived-controls.generated.ts` before `defineConfig` (same synchronous-Node-side-effect pattern already used there for font copying), which `catalog/lib/registry.ts` imports and merges with each story's `controlOverrides`/`excludeControls` to build the final per-story `Controls`.
- `apps/extension/catalog/lib/derive-controls.test.ts` calls `deriveControls()` directly against each primitive's source (read via `import.meta.glob(..., { query: '?raw' })`, same technique `registry.ts` already uses for the source-view panel) — it does not depend on the generated file having been built, so the guard is authoritative even if codegen is stale or skipped.

...
- [x] 9a.7 Animate the card grid: each cell wraps `FolderPageItem` with `animate:flip` + `transition:fade`; durations from a `prefers-reduced-motion`-aware `CARD_MOTION_MS` (0 when reduced). `vitest.setup.ts` stubs `Element.prototype.animate` (jsdom lacks WAAPI); page tests report reduced motion for deterministic counts.

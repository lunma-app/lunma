...
**D3 — "Waiting on you" derived from the existing query axis + CI tone; no new data.** Lane membership = the union of: (a) changes with `relationOf(query) === 'waiting'` (review-requested of me) → reason "review requested"; (b) changes with `relationOf === 'authored'` and `ciLight().tone === 'failing'` → reason "CI failing" (`--danger`); (c) tickets with `query === 'assigned'` not in a done status → reason "assigned to you". Ordered (a) → (b) → (c), capped at **6** (overflow count shown). Items remain in their home section (the lane is a focus overlay, not a move). *Alternative rejected:* add an explicit `actionable` flag to the data layer — unnecessary, the query axis already encodes intent.
...
**D7 — Quiet feed; List capped single column, Grid unchanged.** Article thumbnails sit at `opacity: 0.62; filter: saturate(0.7)` at rest and animate to full on `:hover`/`:focus-visible`. **List** is a single column capped to a comfortable measure (~900px) — it is *not* multi-column, because **Grid** already is the responsive multi-column browse view (kept as-is, gaining only the quiet-thumbnail treatment on its hero images). *Alternative rejected:* 2-up List columns — duplicates Grid and erases the List/Grid distinction (maintainer call).

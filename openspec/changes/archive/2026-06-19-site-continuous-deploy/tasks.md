...
- [x] 3.1 New workflow file, **separate from `ci.yml`** (design D1). Triggers:
  `on: push` with `branches: [main]` — **`main` only, no per-branch previews**
  (previews publish public, never-cleaned `*.pages.dev` URLs; local `preview`
  covers the need); **no `pull_request` trigger** (design D3 — keeps the token
  off fork PRs). Top-level `permissions: { contents: read }` (the deploy auth is the

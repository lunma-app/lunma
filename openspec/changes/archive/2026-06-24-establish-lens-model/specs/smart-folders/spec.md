...
**Reason**: The `smart-folders` capability is renamed to `lenses` by establish-lens-model. Every requirement moves to the new `lenses` capability with lens vocabulary (see this change's `specs/lenses/spec.md`); the `smart-folders` capability is retired.

...
- **Codegen wired into the catalog's existing build lifecycle.** `derive-controls.ts`'s output feeds a generated, gitignored module, written by `apps/extension/catalog/lib/generate-derived-controls.ts` (mirrors the existing font-copy pattern), that `catalog/lib/registry.ts` imports and merges with each story's `controlOverrides`/`excludeControls` to produce the final `Controls` passed to `Story.svelte`. Called from both `vite.catalog.config.ts` (dev-server boot, `catalog:build`) and `vite.config.ts` (plain `vitest run`, since the drift guard's story-module glob transitively reaches `registry.ts` — see design.md's agreed deviation), so it can't go stale or missing between runs.
...
- **New files:** `apps/extension/catalog/lib/derive-controls.ts` (the parser/derivation function), `apps/extension/catalog/lib/derive-controls.test.ts` (unit tests + the drift guard), `apps/extension/catalog/lib/generate-derived-controls.ts` (the codegen write step, shared by both Vite configs), a gitignored generated module (`apps/extension/catalog/lib/derived-controls.generated.ts`, analogous to `catalog/public/fonts/`).
...
- **Modified files:** `apps/extension/catalog/lib/story.ts`, `apps/extension/catalog/lib/registry.ts` (merge derived + overrides), `apps/extension/catalog/lib/controls.ts` (keep `ControlType`/`ControlDef`/`Args`/`defaultArgs`, drop the now-obsolete hand-authoring assumption in its doc comment), `apps/extension/vite.catalog.config.ts` (codegen step, via `generate-derived-controls.ts`), `apps/extension/vite.config.ts` (widen vitest `test.include` to `catalog/**/*.test.ts`, and also call `generateDerivedControls()` — both agreed deviations, see design.md — otherwise the guard never runs under `pnpm verify`, or fails to resolve its own transitive import), all ~35 `apps/extension/catalog/stories/ui/*.stories.svelte`.

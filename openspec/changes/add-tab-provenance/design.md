...
### Decision 8: persisted slice + v19, keyed by tabId
...
Migration is the standard shape: v19 adds the slice, defaulting to `{}`.
...
v18 → v19 adds `provenanceByTabId`, defaulting to `{}`. No data reshaping; the
...
### Spike 3 (added 2026-08-25): no durable tab identity survives a restore

Asked because persisting the edges ourselves *looks* like it should defeat Spike
2 — if the edge is in our store, who cares that `openerTabId` decayed? The answer
is that the edge is keyed by `tabId`, and the identity is what dies, not the
record. Probed whether `chrome.sessions` (or any field on `tabs.Tab`) offers a
handle that survives, so edges could be re-keyed WITHOUT guessing.

Measured with `sessions` temporarily in `permissions`:

- **`tabs.Tab.sessionId` is `null` on every live tab**, before and after restore.
  It is populated only on sessions-API results, so a live tab offers no handle.
- **Restored tabs get fresh ids and no opener.** `100953301/100953302`
  (child → opener parent) came back as `100953367/100953368`, opener `null`.
- **`sessions.getRecentlyClosed()` does return sessionIds** for the closed
  window's tabs — `100953371/372/373` — but they are a THIRD id space, matching
  neither the pre-restore nor the post-restore tab ids. They are handles for
  `sessions.restore()`, not identity for a live restored tab.
- `sessions.getDevices()` → 0 (no sync in the harness; irrelevant to local restore
  either way).

**Conclusion: persistence cannot rescue lineage across a browser restart.** The
only bridge left is URL + tab order, which is exactly the plausible-but-wrong
matching Decision 5 rejects — and it would fire at the moment the user is judging
whether to trust the indentation. Spike 2's verdict stands unchanged, and
`add-tab-provenance-backfill` (real edges from `chrome.history` + `getVisits()`)
remains the only honest route to durability.
...
promoted. It cannot be committed as-is: it needs `webNavigation` in
`manifest.json` `permissions`, which this change only adds as an *optional*
permission in §2 — a committed spec would fail on `main` today. The Spike 1
finding it encodes (an external handoff commits as `start_page`, not `link`) IS a
load-bearing Chrome behaviour whose silent change would wrongly parent every
external open, so it should be re-added as a real e2e spec once §3 lands and the
permission exists. Recorded here so the follow-up is not lost.

## Open Questions

1. ~~**Spike 1** — what `transitionType` / `transitionQualifiers` does an
   external-app open actually produce?~~ **RESOLVED — the resolution rule stands.**
   Measured 2026-08-25 on macOS via a real warm-start handoff: a second Chrome
   process invoked against the same profile (`<chrome> --user-data-dir=<dir>
   <url>`), which the OS-level path routes to the running instance. The first
   main-frame commit reports:

   | Open | `transitionType` | opener at create |
   | --- | --- | --- |
   | External handoff (warm start) | `start_page` | **present** (the focused tab) |
   | In-page link click (same tab) | `link` | present |
   | `target="_blank"` link | `link` | present |
   | `tabs.create` with `openerTabId` | `link` | present at commit |
   | `tabs.create` with no opener | `link` | none |
   | Address-bar navigation | `typed` | n/a |

   An external handoff is **separable** — `start_page`, not `link` — so Decision
   1's cross-tab branch needs no change. The measurement also independently
   justifies Decision 4: the handoff arrives **with a live `openerTabId`** (the
   previously focused tab), so `openerTabId` alone would confidently parent an
   external open to an unrelated tab. `transitionType` is what prevents that
   fail-wrong, and `transitionType` is what `webNavigation` buys.

   Not measured: cold start. Playwright's `launchPersistentContext` rejects a
   positional URL (`Arguments can not specify page to be opened`), so the
   command-line startup path was unreachable in this harness. The initial
   `about:blank` of a fresh profile also commits as `start_page`, so a cold-start
   handoff landing on `start_page` is the likely-but-unverified case. Marked
   `[inferred]`; re-probe if cold-start attribution ever matters.

2. ~~**Spike 2** — does `openerTabId` survive browser session restore?~~
   **RESOLVED — NO. Provenance is browser-session scoped.** Measured 2026-08-25:
   a parent/child pair created with an explicit `openerTabId`, profile closed,
   relaunched with `--restore-last-session`. Before: child → `opener: <parent
   id>`. After: the same URLs return with **new tab ids and `opener: null`**.

   Consequences, all binding on the rest of this change:
   - The `tab-provenance` spec's browser-session-scoped requirement stands as
     written; it is now measured, not assumed.
   - No boot pass can recover one level of lineage — there is nothing to recover.
     The scope question raised in `tasks.md` 1.5 is therefore moot, not deferred.
   - The options copy MUST state that lineage resets when the browser restarts.
     A user who grants "Read your browsing history" and then finds a flat list
     every morning has been mis-sold.
   - `add-tab-provenance-backfill` is the only route to durability, which makes it
     load-bearing rather than a nice-to-have. See the note in Risks.

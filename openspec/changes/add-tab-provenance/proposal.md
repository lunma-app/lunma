...
- **Provenance is persisted** (`AppState` v18 + migration) — required because the
...
- `spaces-and-tabs`: the temp-tab list acquires an ordering/nesting concept
  ...
  of `tempTabIds`.
- `storage-and-migrations`: `CURRENT_SCHEMA_VERSION` 17 → 18 for the persisted
...
`src/shared/schemas.ts` (v18), `src/shared/migrations.ts`,

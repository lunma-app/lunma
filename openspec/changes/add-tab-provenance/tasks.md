...
- [x] 1.1 Write a throwaway probe spec: `sw.evaluate` a
...
- [x] 1.2 **Spike 1 — what transition does an external-app open produce?** Trigger
...
- [x] 1.3 **Evaluate Spike 1 against the design.** If external opens are
...
- [x] 1.4 **Spike 2 — does `openerTabId` survive browser session restore?**
...
- [x] 1.5 **Evaluate Spike 2.** If openers survive restore, note whether a boot
...
- [x] 1.6 Delete the throwaway probe, or promote it to a committed e2e spec if it
...
- [ ] 3.2 Add `ProvenanceEdgeSchema`; add `AppStateV19Schema` with
  `provenanceByTabId`; freeze `AppStateV18Schema`; point `EnvelopeSchema` at v19;
  bump `CURRENT_SCHEMA_VERSION` to 18 (`src/shared/schemas.ts`).
- [ ] 3.3 Add the v18 → v19 migration defaulting `provenanceByTabId` to `{}`
- [ ] 3.3b Extend the `storage-and-migrations` delta with MODIFIED blocks for
  `Versioned local-storage envelope` (current version 18 → 19) and
  `Append-only migrations list` (seventeen entries → eighteen, ending at 19).
  The delta currently carries only ADDED requirements, so archiving it would
  leave the living spec asserting version 18 while the code is at 19 —
  exactly the drift the v17 repair in `remap-renamed-lucide-icons` had to fix.

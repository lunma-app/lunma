...
- `optional_permissions` SHALL include `history`, `bookmarks`, and
  `webNavigation`. `webNavigation` is required by the `tab-provenance`
  capability and by nothing else: `transitionType` is the only signal in the
  extension API surface that separates a real link-follow from `openerTabId`'s
  noise, and without it tab lineage cannot be derived honestly. It SHALL NOT be
  requested at install, and SHALL be requested only on the user gesture that
  enables the `trackTabProvenance` setting.
...
- **THEN** `history`, `bookmarks`, and `webNavigation` SHALL appear under
  `optional_permissions` and SHALL NOT be granted until requested
- **AND** the connector hosts SHALL appear under `optional_host_permissions`

#### Scenario: webNavigation is not requested until provenance is enabled

- **WHEN** a user installs the extension and never enables `trackTabProvenance`
- **THEN** `webNavigation` SHALL NOT be granted
- **AND** no `chrome.webNavigation` listener SHALL be registered
...
### Requirement: The OptionalApiPermission union covers webNavigation

`OptionalApiPermission` in `apps/extension/src/shared/permissions.ts` SHALL be
`'history' | 'bookmarks' | 'webNavigation'`, so `hasApiPermission()`,
`requestApiPermission()`, and `onPermissionsChange()` gate `webNavigation`
through the same foundation module as every other optional API permission. No
call site SHALL touch `chrome.permissions` for `webNavigation` directly.

#### Scenario: webNavigation is requested through the foundation module

- **WHEN** the `trackTabProvenance` toggle requests `webNavigation`
- **THEN** it SHALL call `requestApiPermission('webNavigation')`
- **AND** SHALL NOT call `chrome.permissions.request` directly

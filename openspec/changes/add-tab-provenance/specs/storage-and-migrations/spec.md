...
### Requirement: AppState v18 adds the provenance slice

`CURRENT_SCHEMA_VERSION` SHALL advance from 17 to 18. `AppStateV18Schema` SHALL
add `provenanceByTabId: { [tabId: number]: ProvenanceEdge }`, and
`AppStateV17Schema` SHALL be retained frozen alongside every other historical
version. `EnvelopeSchema` SHALL carry `AppStateV18Schema`.

The v17 → v18 migration SHALL default `provenanceByTabId` to `{}`. No existing
data is reshaped.
...
- **THEN** it SHALL migrate to v18 with `provenanceByTabId` set to `{}`
- **AND** no other slice SHALL change

#### Scenario: A downgrade is still detectable

- **WHEN** a v18 state is read by a build whose `CURRENT_SCHEMA_VERSION` is 17

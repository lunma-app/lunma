...
allowed-tools: Bash(git status*) Bash(pnpm verify) Bash(pnpm build) Bash(node -p *) Bash(mkdir -p release) Bash(zip *) Bash(cd dist*)
---
...
4. Read the version: `node -p "require('./public/manifest.json').version"`.
5. Zip from inside `dist/` so `manifest.json` sits at the archive root (the Web
   Store requires this):
   `mkdir -p release && (cd dist && zip -r -X "../release/lunma-<version>.zip" .)`

...
test.fixme('two Spaces sharing tabs at boot resolve to distinct groups after the reconcile', async ({
  ...
  const ctx = await page.evaluate(async () => {
    ...
    if (!firstSpace) throw new Error('expected a booted Default space');
    const s1 = firstSpace.id; // the booted Default
    const s2 = 'e2e-beta';
  ...
  if (!i1 || !i2) throw new Error('both space instances must survive boot');

...
// smart-folder-page (task 9.2): the real-browser smoke for the full-page folder
...
//
// Like smart-folder-bindings.spec.ts, the runtime `smartFolders` slice is never
// persisted, so the folder is created through the real `createSmartFolder` bus
...
/** The active Space id + the smart folder ids, read straight from `lunma.state`. */
...
test('the smart folder page loads at its URL and renders the folder section + card', async ({
  context,
  page,
  extensionId,
}) => {
  ...
  // Create a smart folder through the real bus command — its immediate fetch hits
  ...
  await page.goto(
    `chrome-extension://${extensionId}/src/launcher/folderpage/index.html?folderId=${folderId}`,
  );

  // The page mounts, mirrors SW state, and renders the folder + its one section.
  await expect(page.getByTestId('folderpage-root')).toBeVisible();
  await expect(page.getByTestId('folderpage-name')).toHaveText('E2E page queue');
  await expect(page.getByTestId('folderpage-section')).toHaveCount(1);
  // The mocked MR rendered as a result card — proof the live runtime reached the page.
  await expect(page.getByTestId('folderpage-item')).toContainText('E2E page merge request');
});

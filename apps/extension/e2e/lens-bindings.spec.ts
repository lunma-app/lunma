...
// lens-item-bindings (task 5.2): the real-browser activation path with
// the built extension loaded — the durable substitute for the dev-load eyeball.
// A lens result row now activates like a pinned tab: click → a bound,
...
//
// The runtime `lenses` slice is never persisted, so items can't be seeded
// through `lunma.state`; instead the forge REST endpoint is mocked via
// `context.route` (it intercepts the service worker's own fetch), and // folder is created through the real `createLens` bus command so the
// engine's immediate fetch lands actual result items. The persisted ids-only
// `lensItemBindings` slice is read straight from storage to assert binding.
//
...
// covered in `sidebar/Lens.test.ts`.
...
/**
 * Open the lens's kebab menu and wait until its Delete entry is really
 ...
 * from `lunma.state` (the ids-only `lensItemBindings` slice IS persisted). */
async function snapshot(page: Page): Promise<{
  spaceId: string | null;
  folderIds: string[];
  // The per-(item, window) slot is `{ tabId, allowGlob }` (lens-tab-boundary).
...
test('a lens row activates like a pinned tab: open bound, re-click focuses, delete demotes', async ({
  context,
  page,
  extensionId,
}) => {
  ...
  // --- create a lens through the real bus command ------------------
  ...
  // The lens node persists; grab its id once it appears.
  ...
  // Delete is a two-step confirm (lens-delete-confirm): the first

...
function existingNode(overrides: Partial<LensNode> = {}): LensNode {
  return {
    kind: 'lens',
    lensKind: 'general',
    id: 'sf-1',
...
describe('LensEditor — create mode', () => {
  ...
  test('confirm dispatches createLens with the panel values (incl. maxItems) and calls onDone', async () => {
    const onDone = vi.fn();
    const { container } = render(LensEditorHarness, { props: { spaceId: 'work', onDone } });
...
describe('LensEditor — RSS adaptation (rss-connector)', () => {
  ...
  test('confirm dispatches createLens with source rss, the chosen maxItems, and NO query', async () => {
    const onDone = vi.fn();
    const { container } = render(LensEditorHarness, { props: { spaceId: 'work', onDone } });
    await pickCardSource(card(container), 'rss');
...
describe('LensEditor — edit mode', () => {
  ...
  test('a maxItems change confirms as updateLens carrying the new cap', async () => {
    const { container } = render(LensEditorHarness, {
      props: { spaceId: 'work', node: existingNode() },
    });
    await pickSelect(container, 'smart-folder-max-items', '50');
  ...
  test('editing a card filter in place dispatches updateLens with the new query', async () => {
    const onDone = vi.fn();
    const { container } = render(LensEditorHarness, {
  ...
  test('changing a card source in place dispatches updateLens (URL preserved when non-default)', async () => {
    const { container } = render(LensEditorHarness, {
      props: { spaceId: 'work', node: existingNode() },
    });
    await pickCardSource(card(container), 'github');
...
describe('LensEditor — multi-filter / multi-source', () => {
...
describe('LensEditor — host-permission request on confirm (least-privilege-permissions D4)', () => {

...
type AddSourceType = LensProvider | 'opml';
...
const DEFAULT_BASE_URL: Record<LensProvider, string> = {
...
const HINT: Record<LensProvider, string> = {
  gitlab:
...
function queryOptionsFor(s: LensProvider): Array<{ value: LensQuery; label: string }> {
  return [
    ...
    : [
        {
          ...
          queries: ['review-requested'] as LensQuery[],
          name: '',
          file: null,
        },
      ],
...
const hint = $derived.by(() => {
  ...
  if (real.length === 1 && real[0]) return HINT[real[0].source as LensProvider];
...
function toggleQuery(index: number, q: LensQuery): void {
...
function dedupedSources(): LensSource[] {
  const byKey = new Map<string, LensSource>();
...
function confirm(): void {
  ...
  if (node) {
    dispatch({ kind: 'updateLens', payload: { ...payloadBase, folderId: node.id } });
  } else {
    dispatch({ kind: 'createLens', payload: payloadBase });
  }

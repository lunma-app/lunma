 ...
 * `[data-tint=…]` / `[data-show-glares=…]` glass overrides apply. FolderPage
...
async function boot(): Promise<void> {
  ...
  // Read the saved tint so it seeds FolderPage's prop (first-paint aurora
  ...
  try {
    ...
    log.error('folderpage boot failed: getCurrentWindowId', { err });
    mount(FolderPage, {
      target,
  ...
  for (let attempt = 0; attempt <= BOOT_RETRY_DELAYS_MS.length; attempt++) {
    try {
      ...
      if (attempt < BOOT_RETRY_DELAYS_MS.length) {
        log.debug('folderpage boot retry', { attempt: attempt + 1, err });
        await delay(BOOT_RETRY_DELAYS_MS[attempt] ?? 800);
      }
    }
  }

  // Stop our boot-window capture; FolderPage installs its own live subscription.
  captureBroadcast();

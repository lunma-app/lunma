 ...
 * - `smartFolder` — a smart node's `FolderId` ↦ its name.
 * - `smartFolderSpace` — a smart node's `FolderId` ↦ the `SpaceId` whose pinned
 *   tree holds it (the smart item's owning Space, for the current-Space scope,
 ...
 * `savedTabFolder` and `smartFoldersProvider` reads `smartFolder` +
 * `smartFolderSpace`, setting `folderName`/`spaceId` on the results they emit (see
...
export interface FolderNameIndex {
  ...
  smartFolder: Record<FolderId, string>;
  smartFolderSpace: Record<FolderId, SpaceId>;
}
 ...
 * `smart` node's id to its name + owning Space. `tab` nodes (and favicon-row
...
export function buildFolderNameIndex(pinnedBySpace: Record<SpaceId, PinNode[]>): FolderNameIndex {
  ...
  const smartFolder: Record<FolderId, string> = {};
  const smartFolderSpace: Record<FolderId, SpaceId> = {};
  for (const [spaceId, nodes] of Object.entries(pinnedBySpace)) {
    for (const node of nodes) {
      if (node.kind === 'folder') {
        ...
        smartFolder[node.id] = node.name;
        smartFolderSpace[node.id] = spaceId;
      }
    }
  }
  return { savedTabFolder, smartFolder, smartFolderSpace };
}

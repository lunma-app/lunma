 ...
 * **every** folder's items across **all** sections (sections keyed by sourceKey
 * `${source}:${host}`) — each item is link-shaped (`{ id, title, url, status? }`)
 * and becomes a result with `source: 'smart'`, `id: 'smart:<namespacedId>'`,
    ...
    for (const [sectionKey, section] of Object.entries(runtime.sections)) {
      for (const item of section.items) {
        ...
        const result: LauncherResult = {
          // Namespaced id matches the `openSmartItem` handler's expected format.
          id: `smart:${sectionKey}:${item.id}`,
          source: 'smart',

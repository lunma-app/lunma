...
// polls). Hence `AppStateV17` (optional slices) rather than the full runtime
// `AppState`. `AppStateV17Schema` is the current persisted schema; v1…v10
...
export type PersistedRead =
  ...
  // `AppStateV17` (every individually-valid Space preserved). The raw payload was
  // still quarantined; this state is written back to self-heal.
  | { kind: 'salvaged'; state: AppStateV17 };
 ...
 */
export function dedupePersistedState(state: AppStateV17): { state: AppStateV17; changed: boolean } {
  let changed = false;
  ...
  const spaceInstancesByWindow: AppStateV17['spaceInstancesByWindow'] = {};
...
/**
 * Recover a valid `AppStateV17` from a payload that failed whole-state validation,
 ...
 *   - The assembled object is re-validated against `AppStateV17Schema`; on success
...
export function salvagePersistedState(migrated: unknown): AppStateV17 | null {
  ...
  const result = AppStateV17Schema.safeParse(assembled);
...
async function writeBackEnvelope(state: AppStateV17): Promise<void> {
  try {
    await chrome.storage.local.set({
      [STATE_STORAGE_KEY]: { schemaVersion: CURRENT_SCHEMA_VERSION, state: toPersistable(state) },
    });
...
export async function readPersistedState(): Promise<PersistedRead> {
  ...
  const parsed = AppStateV17Schema.safeParse(migrated);
  if (!parsed.success) {

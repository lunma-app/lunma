...
import { type AppStateV14, AppStateV14Schema, CURRENT_SCHEMA_VERSION } from '../schemas';
...
// polls). Hence `AppStateV14` (optional slices) rather than the full runtime
// `AppState`. `AppStateV14Schema` is the current persisted schema; v1…v10
...
export type PersistedRead =
  ...
  // `AppStateV14` (every individually-valid Space preserved). The raw payload was
  // still quarantined; this state is written back to self-heal.
  | { kind: 'salvaged'; state: AppStateV14 };
 ...
 */
export function dedupePersistedState(state: AppStateV14): { state: AppStateV14; changed: boolean } {
  let changed = false;
  ...
  const pinnedBySpace: AppStateV14['pinnedBySpace'] = {};
  ...
  const spaceInstancesByWindow: AppStateV14['spaceInstancesByWindow'] = {};
  for (const key of Object.keys(state.spaceInstancesByWindow)) {
    ...
    const nextBySpace: NonNullable<AppStateV14['spaceInstancesByWindow'][number]> = {};
...
/**
 * Recover a valid `AppStateV14` from a payload that failed whole-state validation,
 ...
 *   - The assembled object is re-validated against `AppStateV14Schema`; on success
...
export function salvagePersistedState(migrated: unknown): AppStateV14 | null {
  ...
  const shape = AppStateV14Schema.shape;
  ...
  const result = AppStateV14Schema.safeParse(assembled);
...
async function writeBackEnvelope(state: AppStateV14): Promise<void> {
  try {
    await chrome.storage.local.set({
      [STATE_STORAGE_KEY]: { schemaVersion: CURRENT_SCHEMA_VERSION, state: toPersistable(state) },
    });
...
export async function readPersistedState(): Promise<PersistedRead> {
  ...
  const parsed = AppStateV14Schema.safeParse(migrated);
  if (!parsed.success) {

...
import { AppStateV12Schema } from './schemas';
...
export function onStateBroadcast(handler: (msg: StateBroadcastMessage) => void): () => void {
  const listener = (raw: unknown) => {
    ...
    const stateResult = AppStateV12Schema.safeParse(candidate.state);
...
export async function requestStateSnapshot(): Promise<AppState> {
  ...
  const parsed = AppStateV12Schema.safeParse(msg.state);
  if (!parsed.success) {
...
const LauncherResultSchema = z.object({
  id: z.string(),
  source: z.enum(['tab', 'saved', 'lens', 'bookmark', 'history', 'websearch', 'navigate']),
  title: z.string(),
  url: z.string(),
  score: z.number(),

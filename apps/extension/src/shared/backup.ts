...
import { AppStateV11Schema, BackupEnvelopeSchema, CURRENT_SCHEMA_VERSION } from './schemas';
 ...
 * defaults → `AppStateV11Schema` → `dedupePersistedState`. Returns `{ ok: false }`
...
export function parseBackup(raw: unknown): ParseBackupResult {
  ...
  // the new machine's live tabs on next boot. The AppStateV11Schema `.default({})`
  ...
  const stateParsed = AppStateV11Schema.safeParse(toValidate);

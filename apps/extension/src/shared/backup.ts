...
import { AppStateV17Schema, BackupEnvelopeSchema, CURRENT_SCHEMA_VERSION } from './schemas';
...
export function buildBackup(state: AppState, settings?: Settings): BackupEnvelope {
  const portable: PortableAppState = {
    ...
    spaces: state.spaces,
    // Connected Accounts travel with the backup (connector-accounts) so a
    // restored lens's `sourceId` references resolve. Tokens are NOT included —
    // they live in a separate `chrome.storage.local` key, never in `AppState`.
 ...
 * defaults → `AppStateV17Schema` → `dedupePersistedState`. Returns `{ ok: false }`
...
export function parseBackup(raw: unknown): ParseBackupResult {
  ...
  // the new machine's live tabs on next boot. The AppStateV17Schema `.default({})`
  // on `lensItemBindings` handles that field; the ephemeral slices are optional
  ...
  const stateParsed = AppStateV17Schema.safeParse(toValidate);

...
import { AppStateV18Schema, BackupEnvelopeSchema, CURRENT_SCHEMA_VERSION } from './schemas';
 ...
 * defaults → `AppStateV18Schema` → `dedupePersistedState`. Returns `{ ok: false }`
...
export function parseBackup(raw: unknown): ParseBackupResult {
  ...
  // the new machine's live tabs on next boot. The AppStateV18Schema `.default({})`
  ...
  const stateParsed = AppStateV18Schema.safeParse(toValidate);

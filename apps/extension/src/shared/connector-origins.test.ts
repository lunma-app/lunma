...
import { requiredOriginsForConfig } from './connector-origins';
...
describe('requiredOriginsForConfig', () => {
  ...
  test('GitHub Enterprise is same-origin under the baseUrl', () => {
    expect(requiredOriginsForConfig({ source: 'github', baseUrl: 'https://ghe.acme.com' })).toEqual(
      ['https://ghe.acme.com/*'],
    );
  });
  ...
  test('gitlab / jira / rss each fetch their own baseUrl origin (port preserved)', () => {
    expect(
      requiredOriginsForConfig({ source: 'gitlab', baseUrl: 'https://gitlab.example.com:8443/g' }),
    ).toEqual(['https://gitlab.example.com:8443/*']);
    expect(
      requiredOriginsForConfig({ source: 'jira', baseUrl: 'https://acme.atlassian.net' }),
    ).toEqual(['https://acme.atlassian.net/*']);
    expect(
      requiredOriginsForConfig({ source: 'rss', baseUrl: 'https://blog.example.com/feed.xml' }),
    ).toEqual(['https://blog.example.com/*']);
  });

  test('a malformed baseUrl yields an empty pattern (treated as ungranted), never throws', () => {
    expect(requiredOriginsForConfig({ source: 'gitlab', baseUrl: 'not a url' })).toEqual(['']);
    // github with a malformed baseUrl can't be github.com → falls through to ''.
    expect(requiredOriginsForConfig({ source: 'github', baseUrl: 'not a url' })).toEqual(['']);
  });
});

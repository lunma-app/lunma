...
/**
 * Lenses engine (lenses; connector extraction by github-connector design
 * D1/D2). Everything source-AGNOSTIC lives here:
 *
 *   - scheduling: one repeating alarm (`lunma/lenses-poll`) at the minimum
 *     configured cadence with per-folder due-clocks, plus the parallel
 *     `'lunma/state-request'` refresh-kick listener;
 *   - single-writer discipline: fetches run OFF the coordinator drain and
 *     enqueue `{ source: 'connector', kind: 'lenses.result' }` events; only
 *     the drain's handler writes the runtime slice;
...
export const LENSES_ALARM_NAME = 'lunma/lenses-poll';
 ...
 */
export interface LensesResultEvent {
  source: 'connector';
  kind: 'lenses.result';
...
export interface LensDeps {
  store: LunmaStore;
  enqueue: (event: LensesResultEvent) => void;
}
 ...
 */
export async function fetchLensSectionRuntime(
...
export function collectLenses(state: AppState): LensNode[] {
  const out: LensNode[] = [];
  for (const nodes of Object.values(state.pinnedBySpace)) {
    for (const node of nodes) {
      if (node.kind === 'lens') out.push(node);
    }
  }
  return out;
}
 ...
 * safe and idempotent.
 */
export async function syncLensesAlarm(store: LunmaStore): Promise<void> {
  const folders = collectLenses(store.state);
  if (folders.length === 0) {
    await chrome.alarms.clear(LENSES_ALARM_NAME);
    return;
  }
  const period = Math.max(REFRESH_MINUTES_FLOOR, Math.min(...folders.map((f) => f.refreshMinutes)));
  chrome.alarms.create(LENSES_ALARM_NAME, { periodInMinutes: period });
}
...
export function resetLensesInflight(): void {
  inflight.clear();
}
 ...
 * then fan out one `fetchLensSectionRuntime` per source OFF the drain,
 ...
 * cycle's `ConnectorCaches` (see {@link refreshDueLenses}).
 */
export function startLensRefresh(
  deps: LensDeps,
  node: LensNode,
  caches?: ConnectorCaches,
  only?: ResolvedLensSource[],
    ...
    try {
      await Promise.all(
        sections.map(async (cfg) => {
          const sk = sourceKey(cfg);
          try {
            const runtime = await fetchLensSectionRuntime(cfg, node.maxItems, cycleCaches);
            deps.enqueue({
              source: 'connector',
              kind: 'lenses.result',
            ...
            // fetchLensSectionRuntime never throws by contract; this backstop
...
export function isDue(
  node: Pick<LensNode, 'id' | 'refreshMinutes'>,
  lenses: AppState['lenses'],
  now: number,
): boolean {
  const folder = lenses[node.id];
 ...
 */
export async function refreshDueLenses(deps: LensDeps): Promise<void> {
  const now = Date.now();
  const due = collectLenses(deps.store.state).filter((node) =>
    isDue(node, deps.store.state.lenses, now),
  );
  const caches: ConnectorCaches = new Map();
  await Promise.all(due.map((node) => startLensRefresh(deps, node, caches).completion));
}

/** Handle a fired alarm: on the poll alarm, run the due-check. Mirrors
 * `handleAutoArchiveAlarm` — never mutates the store directly. */
export async function handleLensesAlarm(deps: LensDeps, alarm: chrome.alarms.Alarm): Promise<void> {
  if (alarm.name !== LENSES_ALARM_NAME) return;
  await refreshDueLenses(deps);
}
 ...
 */
export function registerLensesRefreshKick(
  deps: LensDeps,
  ...
  const listener = (raw: unknown, sender: chrome.runtime.MessageSender): undefined => {
    ...
    void whenReady.then(() => refreshDueLenses(deps));
    return undefined;
  };
 ...
 *     refresh (the gate in {@link fetchLensSectionRuntime} now passes for that
 ...
 */
export async function reconcileLensGrants(deps: LensDeps): Promise<void> {
  const now = Date.now();
  const folders = collectLenses(deps.store.state);
  const caches: ConnectorCaches = new Map();
  await Promise.all(
    folders.map(async (node) => {
      const sections = deps.store.state.lenses[node.id]?.sections ?? {};
      ...
      if (anyUnblocked || isDue(node, deps.store.state.lenses, now)) {
        await startLensRefresh(deps, node, caches).completion;
        return;
      }
 ...
 * triggers a full {@link reconcileLensGrants} pass (granting one origin
 ...
 * half-loaded store (mirrors {@link registerLensesRefreshKick}). Returns
 * an unsubscribe.
 */
export function registerLensesPermissionSync(
  deps: LensDeps,
  whenReady: Promise<unknown> = Promise.resolve(),
): () => void {
  return onPermissionsChange(() => {
    void whenReady.then(() => reconcileLensGrants(deps));
  });
}

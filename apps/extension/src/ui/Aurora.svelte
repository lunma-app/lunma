...
/** Tint → overall opacity. `vivid` is the most intense backdrop; the calmer
 * levels fade it back. These values are the JS-side ramp used when `intensity`
 * is passed explicitly (e.g. the new-tab home). Sidebar tint is controlled by
 * CSS `[data-tint]` `--aurora-opacity` overrides in `app.css` (0.35 / 0.16 / 0
 * for vivid / standard / subtle) and does NOT use these constants. `vivid` is
 * capped at 0.5 so `.meta` / `.engine-hint` text on the raw aurora clears
 * WCAG-AA contrast. */

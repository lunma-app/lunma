...
interface ChromeMock {
  ...
  /** chrome.storage.local backing record (the Connectors card + RecentlyArchived). */
...
function installChromeMock(
  ...
  (globalThis as unknown as { chrome: unknown }).chrome = {
    storage: {
      ...
      // the Connectors card reads/writes the `lunma.connectors` record there.
      local: {
...
describe('Options Connectors section (smart-folders, design D10)', () => {
  ...
  test('the intro copy covers both sources (github-connector: card reused, copy widened)', () => {
    const { container } = render(Options, { props: {} });
    const intro = (container.querySelector('.connector-intro')?.textContent ?? '').replace(
      /\s+/g,
      ' ',
    );

# 0009 — Per-window tab bindings (uniform)

- **Status:** Accepted
- **Date:** 2026-06-05
- **Implementing phase:** Phase 3 (see docs/05-roadmap.md) — `per-window-tab-bindings`, consumed by `sidebar-favicon-row` (ADR 0010)
...
Bindings are not persisted across a clean sense — they are rebuilt on restart by
URL match (`tab-bindings.ts` `runRestartRecovery`). Drift (`currentURL !==
originalURL`) is tracked per binding.

Two forces make the single-valued shape inadequate:

1. **The "same Space in multiple windows" promise is half-baked.** CLAUDE.md
   names this the *canonical visual-design opportunity*: the same Space active in
   two windows, each a real place. But a pinned tab shared across both windows
   has **one** binding — so clicking it in the second window focuses the live tab
   in the *first* window (a cross-window focus jump), or the second click
   silently overwrites the first window's binding. The data model can't express
   "this pinned tab is open as a different live tab in each window," which is what
   the two-window case actually is.

2. **The global favicon row (ADR 0010) requires it.** A favorite is shown in
   *every* window across *different* Spaces simultaneously. Clicking it in
   Window-1 (Space A) and Window-2 (Space B) must focus/open a **different** live
   tab in each. With `tabBindings[favId] = tabId` the second window clobbers the
   first. A global favorite is impossible to bind correctly without per-window
   slots.
...
- **D2 — Uniform, not favorites-only.** The per-window shape applies to **all**
  ...
  binding logic per type."* One binding model, no per-type branches. (Decided in
  the 2026-06-05 explore session against the alternative of leaving pinned tabs
  single-bound.)
...
- **D6 — Store stays thin.** The nested map is still chrome-free synchronous
  state; all `chrome.tabs` I/O remains in the coordinator (ADR 0005 D-thin-store,
  ADR 0007 D5). Mutators gain a `windowId` argument; no new async surface in the
  store.
...
- **Favorites-only per-window (keep pinned single-bound).** Smaller blast radius,
  ...
  focus-jump unfixed. Rejected in the explore session.

- **Keep the single binding; accept the multi-window clobber.** Zero refactor,
  but the canonical multi-window-Spaces opportunity stays unrealized and the
  global favicon row (ADR 0010) cannot bind correctly at all. Rejected.
...
- **Persist bindings across restart instead of rebuilding by URL.** Orthogonal to
  ...
  bindings stale on restart (ADR 0007 constraint 3); URL rebuild stays.

## Consequences

- **Direct user value, not just plumbing.** The same Space active in N windows
  finally gives each window its own live tabs — the canonical opportunity becomes
  real. This is why the change satisfies the user-value policy on its own and is
  not stranded infrastructure; it *also* names `sidebar-favicon-row` (ADR 0010) as

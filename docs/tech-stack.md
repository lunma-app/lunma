...
| CSS | Svelte scoped styles + CSS variables | Per-component isolation; theming via variables; a design-token contract in the shared **`@lunma/tokens`** package (`packages/tokens` — type scale, weights, z-index, focus, press, control sizes, glass/glow/aurora, and the canonical nine-colour Space palette + per-colour on-ink) |

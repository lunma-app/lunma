...
| i18n | `@inlang/paraglide-js` | Compiler-based, tree-shakeable, type-safe messages with ICU plurals and runtime locale switching; SW-safe custom locale strategy |
| Catalog source highlight | `shiki` (dev-only) | Svelte syntax highlighting for the dev-only component catalog's source view; ships nothing in the extension bundle |
...
### Paraglide for i18n, with a service-worker-safe custom strategy

Lunma localizes its UI with `@inlang/paraglide-js` (v2), a *compiler*-based i18n library: `paraglide-js compile` reads per-locale message catalogs (`apps/extension/messages/{locale}.json`, `en` is the source of truth) and emits tree-shakeable, type-safe message functions plus a tiny runtime into `apps/extension/src/shared/paraglide/` — gitignored and regenerated on demand: `paraglideVitePlugin` (in `vite.config.ts`) compiles it automatically for `dev`/`build`/`vitest`, and a `postinstall` hook (`pnpm gen:i18n`) seeds it once after `pnpm install` so a cold `typecheck`/`check` — the two scripts that run outside Vite — work without a manual step. Excluded from Biome. Living under `shared/`, the generated runtime imports nothing from other layers, so the import DAG stays legal; `SupportedLocale` derives from its `locales` constant so the locale *set* is single-sourced.

The alternatives — `svelte-i18n` (heavier runtime store, weaker types) and `typesafe-i18n` (maintenance concerns) — lose on type-safety or footprint. Native `chrome.i18n` *can't* satisfy the in-app requirement (it follows the browser UI language with no runtime switch and weak ICU/plural support), so it is used only where it is the *only* option: the manifest / store listing (`default_locale` + `__MSG_*__` + `public/_locales/`).
...
### Homegrown component catalog over Storybook/Histoire

The dev-only component catalog (`apps/extension/catalog/`) — a browsable matrix of every `src/ui` primitive against the live aurora/glass backdrop — is homegrown rather than Storybook or Histoire. Histoire was the requested tool but is incompatible with the pinned stack: `histoire@1.0.0-beta.1` peers on `vite ^7.3.0` (no Vite 8) and `@histoire/plugin-svelte` peers on `svelte ^3 || ^4` (no Svelte 5 runes). Storybook supports the stack but is heavy (manager/preview build, large dep tree) for a private dev surface. The homegrown catalog is ~8 small files, adds **zero** runtime dependencies (it reuses the already-present `vite`, `svelte`, `@sveltejs/vite-plugin-svelte`, `stylelint`, `@biomejs/biome`), auto-discovers `*.stories.svelte` via `import.meta.glob`, and renders primitives inside the real `@lunma/tokens` recipes. Each story renders through a catalog-owned `Story` layout: a live preview bound to editable **controls** (a hand-authored `meta.controls` schema — Svelte 5 has no runtime prop metadata), an **API table** derived from that schema, the curated examples matrix, and a collapsible **source view** (the story's own `?raw` source, syntax-highlighted with `shiki` — the catalog's one dev-only dependency, dual-theme so it follows the toggle). The toolbar also exposes a light/dark theme toggle (the only place outside the immersive controls the catalog reaches into `@/shared` — the shared `applyThemeToDocument` helper) and the dev server runs on port `6006` (distinct from the extension's `5173`). It is **dev-only** and ships **nothing** in the MV3 bundle: a sibling `vite.catalog.config.ts` (`root: catalog/`, no `@crxjs/vite-plugin`) builds it, never referenced by `manifest.json` or `vite.config.ts`'s rollup input. It runs the parallel `verify:catalog` gate (typecheck against `tsconfig.catalog.json`, Biome, svelte-check, stylelint), folded into `verify`.

Each story also carries a hand-authored `controls` schema (`catalog/lib/controls.ts`) that the catalog-owned `catalog/lib/Story.svelte` renders as live editable inputs bound to a preview, an API table (prop · type · default · description), the curated examples matrix, and a collapsible **source view** of the story's own `?raw` source. Svelte 5 exposes no runtime prop metadata, so the schema is hand-authored (it covers the controllable scalar/enum/boolean/text props) rather than auto-introspected.
...
| `@inlang/paraglide-js` | 2.x | Compiler-based i18n (devDependency, `catalog:`). Generates `src/shared/paraglide/` (gitignored, Biome-excluded) via `paraglideVitePlugin`/`postinstall`; SW-safe `custom-lunmaSettings` strategy in `src/shared/i18n.ts` |
